...
"""
train_unet_rgb.py
Train U-Net to predict ab channels from L channel (Lab space) 
using lightweight augmentation. GPU 4-6GB có thể chạy.

Folder structure:
  train/
    L/      -> grayscale images
    RGB/    -> color images (same filenames as L)
  val/
    L/
    RGB/

Usage:
  python train_unet_rgb.py --train_dir ./train --val_dir ./val --epochs 60 --batch_size 8 --img_size 256
"""
...
class RGBColorDataset(Dataset):
    def __init__(self, folder_L, folder_RGB, img_size=256, augment=False):
        ...
        # chỉ duyệt folder_L, lấy tất cả ảnh
    ...
    def __getitem__(self, idx):
        ...
        fRGB = os.path.join(self.folder_RGB, name)  # path ảnh màu tương ứng

        L = Image.open(fL).convert('L').resize((self.img_size, self.img_size), Image.BICUBIC)
        RGB = Image.open(fRGB).convert('RGB').resize((self.img_size, self.img_size), Image.BICUBIC)

        L_np = np.array(L).astype('float32') / 255.0
        ...
        if self.augment and np.random.rand() < 0.5:
            L_np = np.fliplr(L_np).copy()
            ab = np.fliplr(ab).copy()
...
def lab_to_rgb(L_tensor, ab_tensor):
    # L_tensor: [B,1,H,W], ab_tensor: [B,2,H,W]
...
# Inference
# ---------------------------
def infer(model, device, L_path, out_path, img_size=256):
    model.eval()
    L = Image.open(L_path).convert('L').resize((img_size,img_size), Image.BICUBIC)
    L_np = np.array(L).astype('float32')/255.0
...
def main():
    ...
    if args.mode=='train':
        ...
        if args.infer_L:
            infer(model, device, args.infer_L, args.infer_out, img_size=args.img_size)
